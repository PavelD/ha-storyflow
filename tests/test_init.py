...
async def test_async_setup_entry_creates_entities(hass: HomeAssistant):
    ...
    with patch("custom_components.storyflow.StorageHandler") as mock_storage, \
         patch("custom_components.storyflow.StoryManager") as mock_manager, \
         patch("custom_components.storyflow.async_setup_services") as mock_services:
...
async def test_async_setup_entry_uses_persisted_story_id(hass: HomeAssistant):
    ...
    with patch("custom_components.storyflow.StorageHandler"), \
         patch("custom_components.storyflow.StoryManager") as mock_manager, \
...
async def test_async_unload_entry_cleans_up(hass: HomeAssistant):
    ...
    with patch("custom_components.storyflow.StorageHandler"), \
         patch("custom_components.storyflow.StoryManager") as mock_manager, \
         patch("custom_components.storyflow.async_setup_services"), \
         patch("custom_components.storyflow.async_unload_services") as mock_unload:
...
async def test_multiple_entries_same_hass_data(hass: HomeAssistant):
    ...
    with patch("custom_components.storyflow.StorageHandler"), \
        ...
        assert hass.data[DOMAIN][entry1.entry_id] is not hass.data[DOMAIN][entry2.entry_id]
